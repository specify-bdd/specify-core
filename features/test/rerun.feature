Feature: Rerun Failed Tests
    ...
    Background:
        ...
        And a CLI shell
        And a temp file path referenced as "file.rerun.path" # generates a unique temp file path and stores it as ref
    ...
    Rule: Every test run creates a rerun file
        ...
        Scenario: Passing test run creates an empty rerun file
            When a user runs the command "npx specify test --rerun-file=${ref.file.rerun.path} ./binary/passing.feature"
            Then the ${ref.file.rerun.path} file path should exist
            And the ${ref.file.rerun.path} file content should be empty

        @skip @todo
        Scenario: Failing test run creates a rerun file
            When a user runs the command "npx specify test --rerun-file=${ref.file.rerun.path} ./binary/failing.feature"
            Then the ${ref.file.rerun.path} file path should exist
            And the ${ref.file.rerun.path} file content should match ${ref.file.rerun.content.failingScenarios}

    Rule: Only failing tests are included in the rerun file

        @skip @todo
        Scenario: Mixed test run creates rerun file with only failures
            When a user runs the command "npx specify test --rerun-file=${ref.file.rerun.path} ./binary/passing.feature ./binary/failing.feature"
            Then the ${ref.file.rerun.path} file content should match ${ref.file.rerun.content.failingScenarios}

    Rule: Rerun only executes the tests that failed in the last test run

        @skip @todo
        Scenario: Rerun only one scenario in failing.feature
            Given that the ${ref.file.rerun.path} file content is "binary/failing.feature:3"
            When a user runs the command "npx specify test --rerun --rerun-file=${ref.file.rerun.path}"
        ...
        Scenario: Rerun all three scenarios in failing.feature
            And that the ${ref.file.rerun.path} file content is "binary/failing.feature:3:7:11"
            When a user runs the command "npx specify test --rerun --rerun-file=${ref.file.rerun.path}"
    ...
    Rule: Rerun does nothing if the last test run passed completely
        ...
        Scenario: Rerun after passing test run returns an error
            Given that the ${ref.file.rerun.path} file content is empty
            When a user runs the command "npx specify test --rerun --rerun-file=${ref.file.rerun.path}"
